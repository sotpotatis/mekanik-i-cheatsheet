...
\section[title]{Kraftigt \footnote{pun intended i rubriken!}reducerade kraftsystem}
...
\subsubsection{Impulsmomentlagen}
Med avseende på origo gäller
$$
\dot H_O = M_O
$$
\subsection{Impuls}
Impuls är \textit{ändringen av rörelsemängden} vid en viss tidpunkt. Dvs. $$\text{impuls: } I = \vec F\Delta t$$
\subsubsection{Kraftimpulslagen}
\textit{Kraftimpulslagen} relaterar rörelsemängd med impuls.
$$I = \Delta \vec p = m \Delta \vec v = \int_{t_1}^{t_2} \vec F dt$$
...
\begin{ankiflashcard}{Definiera centralrörelse och dess kännetecken.}
    ...
    Principen för en centralrörelse är att \textit{endast en centralkraft\footnote{en kraft som alltid går genom samma punkt}}, inga andra krafter, utför \textit{arbete}. \textit{Exempelvis:} en satellits rörelse runt jorden, en fjäder fäst i en punkt (om alla krafter utom fjäderkraften kan försummas)
...
\end{cases}$$
där $\rho$ är krökningsradie\footnote{``radie av den perfekta cirkeln som passar kring riktningen``}.
...
\subsection{Stötar}
En stöt är en impuls som sker under en (mycket) kort tid. En stöt kan vara \textit{elastisk} (partiklarna rör sig separat från varandra efter kollisionen) eller \textit{inelastisk} (partiklarna rör sig som en sammanhängande massa efter kollisionen).
